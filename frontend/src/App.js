...
import { Shield, Target, Lock, FileText, Users, AlertTriangle, CheckCircle, Database, Activity } from 'lucide-react';
...
export default function App() {
  ...
  const startScan = async () => {
    ...
    setScanning(true); setResults(null); setReport(""); setActiveTab("scan");
    try {
      ...
      setTimeout(() => resultsRef.current?.scrollIntoView({ behavior: 'smooth' }), 100);
        ...
        setScanning(false); 
        const msg = err.response?.data?.details || "Scanning engine unreachable";
        alert(`Error: ${msg}`); 
    }
  };
  ...
  const runRecon = async () => {
    if (!target) return alert("Please enter a target URL");
    setResults(null); setReport(""); setActiveTab("scan");
    try {
        ...
        >
          ...
          <div style={{ background: "rgba(9, 9, 11, 0.7)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "0 40px", display: "flex", alignItems: "center", height: 72, position: "fixed", top: 0, left: 0, right: 0, zIndex: 100 }}>
            ...
            <div style={{ display: "flex", gap: 16, marginRight: 24 }}>
                <button onClick={() => setActiveTab("scan")} style={{ background: "none", border: "none", color: activeTab === "scan" ? "#38bdf8" : "#94a3b8", cursor: "pointer", fontSize: 14, fontWeight: "500" }}>Dashboard</button>
                <button onClick={() => setActiveTab("history")} style={{ background: "none", border: "none", color: activeTab === "history" ? "#38bdf8" : "#94a3b8", cursor: "pointer", fontSize: 14, fontWeight: "500" }}>History</button>
            </div>
